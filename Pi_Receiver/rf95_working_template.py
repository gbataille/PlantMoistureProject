...
def spi_read(register) -> int:
    """
    Returns the content of the requested register, as an int (1 byte)
    """
    send_data = bytes([register & ~SPI_WRITE_MASK, 0x0])
    length, recv_data = wiringpi.wiringPiSPIDataRW(SPIchannel, send_data)
    if length != 2:
        raise ValueError("not enough data returned")
    # The first byte is expected to be a constant mask 0xc0
    return recv_data[1]
...
def spi_burst_read(register, length):
    ...
    for _ in range(0, length):
        d = spi_read(register)
        data.append(d)
        print("Reading 1 byte: ", d)
    return data


def spi_write(register, value):
    send_data = bytes([register | SPI_WRITE_MASK, value])
    length, recv_data = wiringpi.wiringPiSPIDataRW(SPIchannel, send_data)
    return recv_data
...
def get_frequency() -> float:
    """
    Returns the setup frequency in MHz
    """
    ...
    return (msb << 16 | mid << 8 | lsb) * (32e6 / 2**19) / 1000000  # freq * 32 mhz / 2^19


def set_frequency(frequency: float):
    """
    Args:
        * frequency: a frequency value in MHz
    """
    fstep = 32e6 / 2 ** 19
...
if __name__  == "__main__":
    ...
    print(readMode)
    print("current mode", bin(readMode))
    ...
    print(readMode)
    print("current mode", bin(readMode))
    print(readMode & (RF95Modes.sleep | RF95Modes.loRa) == (RF95Modes.sleep | RF95Modes.loRa))
